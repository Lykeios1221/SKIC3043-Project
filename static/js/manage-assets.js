$(document).ready(function() {
  ...
  async function processAssetsSubmitForm(evt, type, form, storage) {
    ...
    if (valid) {
      ...
      drawTableRow(table, rowForm,
          excludeEntryByKey(dataDict, 'email').map(e => e[1]), rowIndex);
    ...
    return `
        <form method="GET" class="${dataType}-row-form">
            ...
            <button type="submit" name="action" value="modify" class="btn btn-secondary m-1"
             ...
             onclick="viewPdf('${dataType}', '${dataDict['id']}')">View</button>
        </form>
    `;
  }

  function excludeEntryByKey(dict, key) {
    return Object.entries(dict).filter(entry => !entry[0].includes(key));
  }
  ...
  $('#commitAssets').on('click', function() {
    ...
    var originalInventories = JSON.parse(
        JSON.stringify(userAssets.inventories));
  ...
  function compareArrays(originalArray, modifiedArray) {
    ...
    for (const modifiedItem of modifiedArray) {
      const originalItem = originalArray.find(
          item => String(item.id) === String(modifiedItem.id));
      if (!originalItem) {
    ...
    // Check for deletions
    const deletions = originalArray.filter(originalItem => !modifiedArray.some(
        modifiedItem => String(modifiedItem.id) === String(originalItem.id)));
  ...
  $('#printAssets').on('click', function() {
    $.ajax({
      ...
      dataType: 'html',
      data: JSON.stringify(
          {'inventory': inventories, 'revenue': revenues, 'expense': expenses}),
      success: function(response) {
        var w = window.open();
